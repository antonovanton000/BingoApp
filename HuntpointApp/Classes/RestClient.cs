...
namespace HuntpointApp.Classes
{
    public class RestClient
    {
        ...
        public async Task<RestResponse<IEnumerable<HuntpointAppPlayer>>> GetAvailablePlayersAsync(string playerId)
            => await GetFromAPI<IEnumerable<HuntpointAppPlayer>>($"huntpoint/players/available/{playerId}");

        public async Task<RestResponse<IEnumerable<HuntpointAppPlayer>>> GetAllPlayersAsync(string playerId)
            => await GetFromAPI<IEnumerable<HuntpointAppPlayer>>($"huntpoint/players/all/{playerId}");


        public async Task<RestResponse<HuntpointAppPlayer>> GetPlayerInfoAsync(string playerId)
            => await GetFromAPI<HuntpointAppPlayer>($"huntpoint/players/info/{playerId}");

        public async Task<RestResponse<UpdateInfo>> GetLastVersionAsync()
            => await GetFromAPI<UpdateInfo>($"huntpoint/app/lastversion");

        public async Task<RestResponse<UpdateInfo>> GetModLastVersionAsync()
            => await GetFromAPI<UpdateInfo>($"huntpoint/mod/lastversion");

        public async Task<RestResponse<RoomConnectionInfo>> GetRoomInfo(string roomId)
            => await GetFromAPI<RoomConnectionInfo>($"huntpoint/room/info/{roomId}");
        
        public async Task<RestResponse<Room>> ConnectToRoom(ConnectRoomModel model)
        {
            try
            {
                var url = $"huntpoint/room/connect";
        ...
        {                        
            var response = await PostAsync($"huntpoint/room/newboard/{roomId}", CancellationToken.None, false);
            if (response?.IsSuccessStatusCode == true)
            {
                var content = await response.Content.ReadAsStringAsync();
                if (content == "success")
        ...
        public async Task<RestResponse<string>> UpdateUserInfoAsync(HuntpointAppPlayer player)
        {
            var json = JsonConvert.SerializeObject(player);
            var response = await PostAsync($"huntpoint/players/signup", json, CancellationToken.None, false);
            if (response?.IsSuccessStatusCode == true)
            {
                var content = await response.Content.ReadAsStringAsync();
                if (content == "success")
        ...
        public async Task<RestResponse<string>> SendGameInvite(HuntpointAppPlayer currentPlayer, HuntpointAppPlayer player, string data)
        {
            ...
            var response = await PostAsync($"huntpoint/game/invite", jobj, CancellationToken.None, false);
            if (response?.IsSuccessStatusCode == true)
            {
                var content = await response.Content.ReadAsStringAsync();
                if (content == "success")
        ...
        public async Task<RestResponse<string>> SendGameResult(GameResult result)
        {
            ...
            var response = await PostAsync($"huntpoint/game/submitResult", json, CancellationToken.None, false);
            if (response?.IsSuccessStatusCode == true)
            {
                var content = await response.Content.ReadAsStringAsync();
                if (content == "success")
        ...
        public async Task<RestResponse<string>> SharePreset(SendPresetModel data)
        {
            ...
            var response = await PostAsync($"huntpoint/preset/share", json, CancellationToken.None, false);
            if (response?.IsSuccessStatusCode == true)
            {
                var content = await response.Content.ReadAsStringAsync();
                if (content == "success")
        ...
        {
            var playerId = HuntpointApp.Properties.Settings.Default.PlayerId ?? "";
            var json = JsonConvert.SerializeObject(new { playerId = playerId });
            var response = await PostAsync($"huntpoint/ping/{playerId}", json, CancellationToken.None, false);
            if (response?.IsSuccessStatusCode == true)
            {
                var content = await response.Content.ReadAsStringAsync();
                if (content == "pong")
        ...
        public async Task<RestResponse<Room>> CreateRoomAsync(NewRoomModel newBoard)
        {
            ...
            var response = await PostAsync($"huntpoint/room/create", json, CancellationToken.None, false);
            if (response?.IsSuccessStatusCode == true)
            {
                var content = await response.Content.ReadAsStringAsync();
                if (content == "error")
